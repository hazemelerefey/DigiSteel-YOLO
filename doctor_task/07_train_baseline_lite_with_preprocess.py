"""
Step 7-LITE: Train Lightweight CNN WITH Preprocessing (CPU Optimized)
Lightweight version optimized for CPU execution
"""
...
class LitePreprocessedTrainer:
    ...
    def load_images_labels_preprocessed(self, split):
        """Load preprocessed images with CLAHE"""
        ...
        print(f"Loading {split} (with CLAHE): {len(image_files)} images...")
        
        for img_file in image_files:
            img = cv2.imread(str(img_file), cv2.IMREAD_GRAYSCALE)
            img = img.astype(np.float32) / 255.0
            
            # Apply CLAHE (fast on CPU)
    ...
    def train(self, epochs=20, batch_size=16):
        ...
        print("\n=== Loading Data (WITH Preprocessing - CLAHE) ===")
        ...
        print("Expected to be 3-5% more accurate than baseline...")
    ...
    def save_results(self, metrics, tag="lite_baseline_with_preprocessing"):
        """Save results"""
        results = {
...
if __name__ == "__main__":
    print("=" * 70)
    print("Step 7-LITE: Train Lightweight CNN (WITH Preprocessing)")
    print("=" * 70)
    
    trainer = LitePreprocessedTrainer(
    ...
    # Save
    trainer.model.save("doctor_task/models/cnn_baseline_lite_with_preprocess.h5")
    trainer.save_results(metrics)
    
    print("\n✓ Training COMPLETE")
    print("✓ Compare accuracy with baseline - should be 3-5% better!")
